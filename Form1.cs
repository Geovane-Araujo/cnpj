...
namespace Cnpj
{
    public partial class Form1 : MetroFramework.Forms.MetroForm
    {
        ...
        private void metroButton4_Click(object sender, EventArgs e)
        {
            ...
            int db = 0;
            if(chekDb.Text == "MySql")
            {
                CnpjMysql(caminhoa, caminhologa);
            }
            else if (chekDb.Text == "Postgres")
            {
                CnpjPostgres(caminhoa, caminhologa);
            }
            else
            {
                db = 3;
        ...
        public static void CnpjPostgres(String caminho, String caminholog)
        {
        ...
        public static void CnpjMysql(String caminho, String caminholog)
        {
            ...
            var connectO = "Persist Security Info=False;server=localhost;database=cnpj;uid=root;pwd=1816";
            var con = new MySqlConnection(connectO);
            var cons = new MySqlConnection(connectO);
            ...
            var sql2 = "INSERT INTO socio(cnpj,Socio) VALUES(@cnpj,@socio)";
            var commands = new MySqlCommand(sql2, cons);
            ...
            while ((linha = file.ReadLine()) != null)
            {
                ...
                if (verificacao == 2)
                {
                    ...
                    try
                    {
                        ...
                        if (cnpjOld == cnpj)
                        {
                            int validador = Util.BuscacnpjMysql(con, cnpj);
                            if (validador == 1)
                            {
                                commands.ExecuteNonQuery();
                                Console.WriteLine("Sócio Cadastrado com Sucesso!!  " + qt);
